...
<footer class="text-center">
    ...
    <script type="text/javascript">
        ...
        {  
			...
			var pswdPattern=/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{5,8}$/;
            if (pss == 0 || pss == '') {
                alert("Enter Password");
                return false;
            }
			if (!pswdPattern.test(pss))
            {
                alert("Must contain at least one number and one uppercase and lowercase letter, and at least 5 ,max 8");
                return false;
            }
            ...
            if(ad == 0 || ad == '')
            {
			 ...
			 var area = document.getElementById('DropDownList1').value;
            if(area == '--Select--')
            {
                alert("Enter Area Dropdown");
                return false;
            }
